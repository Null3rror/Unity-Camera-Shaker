...
{
    public class CameraShakeFadeInState : TimedState<CameraShaker>
    {
        private float _roughness, _magnitude;
        public CameraShakeFadeInState(float time) : base(time)
        {
        }

        public override void OnEnter(CameraShaker cameraShaker)
        {
            time = cameraShaker.Preset.fadeInTime;
        ...
        public override void Update(CameraShaker cameraShaker)
        {
            ...
            cameraShaker.Amount = amount * (NormalizedElapsedTime() * _magnitude);
        }

        public override State<CameraShaker> DetermineNextState(CameraShaker cameraShaker)
        {
            return IsOverTime() ? cameraShaker.Fsm.Shake : base.DetermineNextState(cameraShaker);
        }
    }
}
